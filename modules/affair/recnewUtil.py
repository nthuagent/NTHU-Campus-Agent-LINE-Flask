...
def create_news_list(recnews):  # date有問題
    ...
    for news in recnews:
        ...
        # 檢查是否都有https
        if not i["imgUrl"].startswith("https") and not i["sourceUrl"].startswith(
            "https"
        ):
            continue
        ...
        url = (
            i["imgUrl"]
            or "https://pgw.udn.com.tw/gw/photo.php?u=https://uc.udn.com.tw/photo/2018/09/23/realtime/5303778.jpg&x=0&y=0&sw=0&sh=0&exp=3600"
        )
        url_text = str(i["sourceUrl"]) or "https://reurl.cc/yZLVpq"
        ...
        recnew_text = f"活動類型：{i['category']}\n日期：{date_str}"
        if i["time"]:
            recnew_text = f"{recnew_text}\n時間：{i['time']}"
