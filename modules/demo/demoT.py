...
def demoT_intro():  # DEMO 啟動器
    demoT_intro = TemplateSendMessage(
        ...
        template=ButtonsTemplate(
            ...
            text="請選擇你要使用的 DEMO",
            actions=[
                MessageAction(label="DEMO", text="[示範]DEMO"),
                MessageAction(label="DEMO0", text="[示範]DEMO0"),
            ],
        ),
    )
    return demoT_intro
...
def demoT_intro1():  # DEMO 啟動器
    demoT_intro_1 = TemplateSendMessage(
        ...
        template=ButtonsTemplate(
            ...
            text="請選擇你要使用的 DEMO",
            actions=[
                MessageAction(label="tzaiwu_space", text="[示範]DEMO1"),
                MessageAction(label="bus_route_template", text="[示範]DEMO2"),
                MessageAction(label="ok_button", text="[示範]DEMO3"),
                MessageAction(label="image_temp", text="[示範]DEMO4"),
            ],
        ),
    )
...
def demoT_intro2():  # DEMO 啟動器
    demoT_intro_2 = TemplateSendMessage(
        ...
        template=ButtonsTemplate(
            ...
            text="請選擇你要使用的 DEMO",
            actions=[
                MessageAction(label="qa_info", text="[示範]DEMO5"),
                MessageAction(label="image_map", text="[示範]DEMO6"),
                MessageAction(label="隨機運勢", text="[示範]DEMO7"),
                MessageAction(label="校內工讀", text="[選單]清華校內工讀"),
            ],
        ),
    )
        ...
        template=CarouselTemplate(
            columns=[
                ...
                CarouselColumn(
                    ...
                    actions=[
                        URIAction(
                            label="仁齋空間登記！", uri="https://forms.gle/td3pvqHQopKZmESE6"
                        ),
    ...
    bus_route_template = TemplateSendMessage(
        ...
        template=ButtonsTemplate(
            ...
            actions=[
                ...
                MessageAction(label="校本部公車", text="[公車]校本部公車"),
                MessageAction(label="南大專車", text="[公車]南大專車"),
                MessageAction(label="83路線公車", text="[公車]83公車")
                # # TODO: 動態校車
    ...
    confirm_template_message = TemplateSendMessage(
        ...
        template=ConfirmTemplate(
            ...
            actions=[
                ...
                MessageAction(label="DEMO5", text="[示範]DEMO5"),
            ],
        ),
    )
...
def image_map():
    image_map = ImagemapSendMessage(
        ...
        actions=[
            URIImagemapAction(
                link_uri="", area=ImagemapArea(x=174, y=65, width=707, height=416)
            ),
            MessageImagemapAction(
                text="hello", area=ImagemapArea(x=520, y=0, width=520, height=520)
            ),
        ],
    )
    return image_map
