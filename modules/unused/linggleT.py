...
def timeplace():
    ...
    place_template_1 = LocationSendMessage(
        title="清華教育館", address="清華教育館", latitude=24.795727, longitude=120.993952
    )
    text_template_2 = TextSendMessage(text="8/16(日) 09:30 前於清大計算機中心集合")
    place_template_2 = LocationSendMessage(
        title="清大計算機中心", address="清大計算機中心", latitude=24.794308, longitude=120.993361
    )
...
def traffic():
    ...
    text_template_1 = TextSendMessage(
        text="""1. 高鐵接駁：國光客運182\n
...
3. 計程車：從火車站→清華大學校本部門口，車資約200~250元"""
    ...
    text_template_2 = TextSendMessage(
        text="2. 市區公車：從火車站→清華大學校本部門口\n新竹客運公車號碼 ：藍1區市區公車、2路"
    )
...
def cancel():
    text_template = TextSendMessage(
        text="""1. 因不可抗力或不可歸責之因素而須取消活動，將擇期辦理；擇期無法參加者或是活動無法擇期辦理將全額退費。\n
