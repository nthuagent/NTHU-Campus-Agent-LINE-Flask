                ...
                CarouselColumn(
                    ...
                    actions=[
                        PostbackTemplateAction(
                            label="查看實習需求", data="source=richmenu&flag=list&info=intern"
                        )
                    ],
                ),
                CarouselColumn(
...
def intern_chat_carousel(intern_demands):  # 實習經驗對談列表
    ...
    for block in data_blocks:
        ...
        for item in block:
            ...
            col = CarouselColumn(
                ...
                actions=[
                    PostbackTemplateAction(
                        label="顯示需求詳細資訊",
                        data="source=richmenu&flag=demand&info=intern_chat_content_"
                        + str(demand_id),
                    ),
                    PostbackTemplateAction(
                        label="我要協助",
                        data="source=richmenu&flag=demand&info=intern_chat_confirm_"
                        + str(demand_id),
                    ),
                ],
            )
        ...
        carousel_template = TemplateSendMessage(
            alt_text="實習經驗一對一對談需求列表", template=CarouselTemplate(columns=columns)
        )
...
def traffic_chat_carousel(intern_demands):  # 實習經驗對談列表
    ...
    for block in data_blocks:
        ...
        for item in block:
            ...
            col = CarouselColumn(
                ...
                actions=[
                    PostbackTemplateAction(
                        label="顯示需求詳細資訊",
                        data="source=richmenu&flag=demand&info=traffic_chat_content_"
                        + str(demand_id),
                    ),
                    PostbackTemplateAction(
                        label="我要協助",
                        data="source=richmenu&flag=demand&info=traffic_chat_confirm_"
                        + str(demand_id),
                    ),
                ],
            )
        ...
        carousel_template = TemplateSendMessage(
            alt_text="交通資訊需求列表", template=CarouselTemplate(columns=columns)
        )
        demands_list.append(carousel_template)
            ...
            items=[
                QuickReplyButton(
                    ...
                    action=PostbackAction(
                        label="沒錯",
                        data="source=richmenu&flag=demand&info=intern_chat_help_"
                        + str(demand_id),
                    ),
                ),
                QuickReplyButton(
        ...
        quick_reply=QuickReply(
            items=[
                QuickReplyButton(
                    ...
                    action=PostbackAction(
                        label="沒錯",
                        data="source=richmenu&flag=demand&info=traffic_chat_help_"
                        + str(demand_id),
                    ),
                ),
                QuickReplyButton(
        ...
        quick_reply=QuickReply(
            items=[
                QuickReplyButton(
                    ...
                    action=PostbackAction(
                        label="沒錯",
                        data="source=richmenu&flag=demand&info=demand_complete_"
                        + str(demand_id),
                    ),
                ),
                QuickReplyButton(
        ...
        for item in block:
            ...
            if len(item["match_info"]) != 0 and item["status"] != 0:
                ...
                action_list = [
                    PostbackTemplateAction(
                        label="顯示需求詳細資訊",
                        data="source=richmenu&flag=demand&info=published_demand_content_"
                        + str(demand_id),
                    ),
                    URITemplateAction(label="互動留言版", uri=demand_url),
                    PostbackTemplateAction(
                        label="完成需求",
                        data="source=richmenu&flag=demand&info=demand_complete_confirm_"
                        + str(demand_id),
                    ),
                ]
            else:
                action_list = [
                    ...
                    PostbackTemplateAction(
                        label="互動留言版", data="source=richmenu&flag=demand&info=no_url"
                    ),
                    PostbackTemplateAction(
                        label="完成需求", data="source=richmenu&flag=demand&info=no_match"
                    ),
                ]

            col = CarouselColumn(
                title=demand_title, text=demand_desc, actions=action_list
            )
            columns.append(col)

        carousel_template = TemplateSendMessage(
            alt_text="刊登紀錄需求列表", template=CarouselTemplate(columns=columns)
        )
        demands_list.append(carousel_template)
...
def helped_demands(helped_demands):  # 個人協助需求
    ...
    for block in data_blocks:
        ...
        for item in block:
            ...
            col = CarouselColumn(
                ...
                actions=[
                    PostbackTemplateAction(
                        label="顯示需求詳細資訊",
                        data="source=richmenu&flag=demand&info=helped_demand_content_"
                        + str(demand_id),
                    ),
                    URITemplateAction(label="互動留言版", uri=demand_url),
                ],
        ...
        carousel_template = TemplateSendMessage(
            alt_text="協助紀錄需求列表", template=CarouselTemplate(columns=columns)
        )
        demands_list.append(carousel_template)
        ...
        "body": {
            ...
            "contents": [
                ...
                },
                {
                    ...
                    "contents": [
                        {"type": "text", "text": "需求編號："},
                        {"type": "text", "text": demand_id, "align": "end"},
                    ],
                    "margin": "lg",
                },
                ...
                {
                    "type": "button",
                    "action": {"type": "uri", "label": "互動留言板", "uri": doc_url},
                    "offsetTop": "md",
                    "margin": "md",
                    "adjustMode": "shrink-to-fit",
                    "style": "secondary",
...
def match_fail_result(demand_type, demand_id):  # 需求匹配失敗
    ...
    """.format(
        demand_type=demand_type, demand_id=demand_id
    )
        ...
        template=CarouselTemplate(
            columns=[
                ...
                CarouselColumn(
                    ...
                    actions=[
                        PostbackTemplateAction(
                            label="我要回饋", data="source=richmenu&flag=point&info=comment"
                        )
                    ],
                ),
            ]
...
def tb_info():
    text_template = TextSendMessage(
        text="""時間銀行的概念是指個體可以將用於解決他人問題的時間，轉換成另一種價值。
            ...
            columns=[
                CarouselColumn(
                    ...
                    actions=[
                        ...
                        PostbackTemplateAction(
                            label="我的徽章", data="source=richmenu&flag=point&info=mybadge"
                        ),
                    ],
                )
            ]
    ...
    for i in all_badges:
        ...
        if i["type"] == "weekly_publish2":  # 第三週挑戰
            ...
            if len(published_demands) == 0:  # 從沒刊登過
                ...
                for j in published_demands:  # 判斷是否有在期間內刊登需求
                    created_time_obj = datetime.strptime(
                        j["created_datetime"], "%Y-%m-%d %H:%M:%S"
                    )
                    if (
                        created_time_obj <= end_time_obj
                        and created_time_obj >= start_time_obj
                    ):
            ...
            if len(helped_demands) == 0:  # 從沒協助過
                ...
                for j in helped_demands:  # 判斷是否有在時間前協助需求
                    created_time_obj = datetime.strptime(
                        j["match_info"]["created_datetime"], "%Y-%m-%d %H:%M:%S"
                    )
                    if (
                        created_time_obj <= end_time_obj
                        and created_time_obj >= start_time_obj
        ...
        badge_json = {
            ...
            "body": {
                ...
                "layout": "vertical",
                "contents": [
                    {"type": "image", "url": img_url, "size": "xl"},
